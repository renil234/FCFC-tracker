...
class AFLLineupClient:
    ...
    def refresh(self, season: int, roster: list[dict[str, str]], previous_statuses: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        club_rosters, errors = self.official_club_team_selections(fixtures, roster)
        statuses = build_player_statuses_from_clubs(
            roster, fixtures, club_rosters, checked_at, previous_statuses
        )
        usable = sum(int(side.get("raw_selected_count", 0)) + len(side.get("emergencies", [])) for side in club_rosters.values())
        if usable <= 0:
            failure = "No usable player lists were found in the official club selection articles. Use the manual overrides in Team Announcements for this round."
        else:
            failure = ""
        return {
            ...
            "lineup_source": "Official AFL club team-selection articles",
        }
