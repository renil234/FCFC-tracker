...
class AFLLineupClient:
    ...
    def refresh(
        ...
        # Primary source: ESPN's public match-summary JSON. Unlike the AFL
        # line-up widget, this is a normal JSON request and needs no browser,
        # proxy, GitHub Action or repository write-back.
        try:
            espn_events = self._espn_scoreboard(fixtures)
        except Exception as exc:
            espn_events = []
            errors.append({"level": "warning", "match_id": "espn", "message": str(exc)})

        for fixture in fixtures:
            match_id = fixture["match_id"]
            try:
                match_rosters[match_id] = self.espn_match_roster(fixture, espn_events)
            except Exception as exc:
                failed.add(match_id)
                errors.append({"level": "warning", "match_id": match_id, "message": f"ESPN: {exc}"})

        # Fallback: try the AFL roster endpoint only for matches ESPN could not
        # validate. This endpoint is not trusted as the sole source because it
        # has previously returned empty bodies and 403 responses.
        if failed:
            try:
                token = self.token()
            except Exception as exc:
            ...
            if token:
                ...
                for fixture in fixtures:
                    ...
                    try:
                        ...
                        if home_count < 18 or away_count < 18:
                            raise RuntimeError(
                                f"incomplete AFL roster ({fixture['home']} {home_count}, {fixture['away']} {away_count})"
                            )
                        match_rosters[match_id] = parsed
                    except Exception as exc:
                        still_failed.add(match_id)
                        errors.append({"level": "warning", "match_id": match_id, "message": f"AFL fallback: {exc}"})
                failed = still_failed
        ...
        return {
            ...
            "lineup_errors": errors,
            "lineup_source": "ESPN public match summaries with AFL roster fallback",
