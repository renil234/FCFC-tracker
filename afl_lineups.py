...
class AFLLineupClient:
    ...
    def refresh(
        ...
        try:
            ...
            # The Streamlit app cannot create the browser-collected file. Keep the
            # last known statuses and return a clear workflow instruction instead
            # of raising a traceback in the user interface.
            fixtures: list[dict[str, Any]] = []
            fixture_error = ""
            try:
                fixtures = self.upcoming_matches(season)
            except Exception as fixture_exc:
                fixture_error = str(fixture_exc)
            errors = [{
                "level": "error",
                "match_id": "browser-cache",
                "message": str(exc),
            }]
            if fixture_error:
                errors.append({
            ...
            return {
                ...
                "lineup_failure_message": (
                    "No validated team-selection file exists yet. Run the GitHub Action "
                    "‘AFL Team Line-ups’ and wait for it to commit data/lineups_latest.json. "
                    "Existing player statuses have not been changed."
                ),
                "lineup_source": "Awaiting AFL Team Line-ups browser workflow",
                "lineup_source_url": AFL_TEAM_LINEUPS_PAGE,
                "lineup_collection_method": "playwright-pending",
            }
