...
def build_statuses_from_central_article(
    ...
    for row in roster:
        ...
        if _name_in_change_text(name, ins, club_players): selected = True
        if _name_in_change_text(name, outs, club_players): selected = False
        if selected:
...
class AFLLineupClient:
    ...
    def central_teams_article(self, round_number: int | None) -> tuple[str, str, dict[str, str], list[dict[str, str]]]:
        ...
        for link in links[:80]:
            try:
                response = self.session.get(link, timeout=self.timeout, headers={"User-Agent": "Mozilla/5.0", "Referer": "https://www.afl.com.au/news"})
                response.raise_for_status()
                ...
                continue
        if not best:
            raise RuntimeError("No current AFL TEAMS article for the upcoming round could be found.")
