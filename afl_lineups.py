...
class AFLLineupClient:
    ...
    def refresh(self, season: int, roster: list[dict[str, str]], previous_statuses: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            # A normal AFL round has 18 teams. Accept a partial page only when it
            # still contains enough clubs to be clearly meaningful.
            if len(club_rosters) >= 10:
            ...
            page_error = f"AFL Team Line-ups page yielded only {len(club_rosters)} club lists"
        except Exception as exc:
            page_error = f"AFL Team Line-ups page: {exc}"
        ...
        if page_error:
            errors.append({"match_id": "page", "level": "warning", "message": page_error})
        ...
        return {
            ...
            "lineup_source": "AFL match-roster API fallback",
        }
