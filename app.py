...
def main() -> None:
    ...
    from afl_lineups import (
        AFLLineupClient,
        STATUS_LABELS,
        eligible_players,
        lineup_check_due,
        load_committed_lineup_result,
        result_from_pasted_text,
    )
    ...
    # Team selections are collected by a scheduled Playwright browser workflow
    # and committed to data/lineups_latest.json. Streamlit only reads the last
    # validated file, so an AFL website failure cannot erase a good round.
    committed_lineup_warning = ""
    try:
        committed_result = load_committed_lineup_result(
            season=season,
            roster=ROSTER,
            previous_statuses=dict(cache.get("team_status") or {}),
        )
        cache.update(committed_result)
        if committed_result.get("afl_next_opponents"):
            cache.setdefault("next_opponents", {}).update(committed_result["afl_next_opponents"])
    except FileNotFoundError:
        committed_lineup_warning = (
            "No validated AFL team-selection file exists yet. Run the GitHub Action "
            "‘AFL Team Line-ups’ so it can create data/lineups_latest.json."
        )
    except Exception as exc:
        committed_lineup_warning = str(exc)
    ...
    with st.sidebar:
        ...
        lineup_refresh_clicked = st.button(
            "Reload saved AFL line-ups", use_container_width=True
        )
        ...
        if cache.get("lineups_refreshed_at"):
            st.caption(f"Line-ups collected: {cache['lineups_refreshed_at']}")
        else:
            st.caption("No validated AFL line-up file has been collected yet.")
            ...
            starter_rows.append(
                {
                    ...
                    "Team status": (statuses.get(row["player"]) or {}).get(
                        "status_label", STATUS_LABELS["not_announced"] if lineup_current else "Not checked"
                    ),
                }
            )
            ...
            roster_rows.append(
                {
                    ...
                    "Team status": status_record.get("status_label", STATUS_LABELS["not_announced"] if lineup_current else "Not checked"),
    ...
    with tabs[3]:
        if cache.get("lineup_source"):
            round_value = cache.get("lineup_round_number")
            source_note = str(cache.get("lineup_source") or "")
            if round_value:
                source_note += f" — Round {round_value}"
            if cache.get("lineups_refreshed_at"):
                source_note += f" — collected {cache['lineups_refreshed_at']}"
            st.caption(source_note)
        if committed_lineup_warning:
            st.warning(committed_lineup_warning)
        status_rows = []
        for player in ROSTER:
            ...
            status_rows.append(
                {
                    "Player": player["player"],
                    "Club": player["club"],
                    "Status": record.get("status_label", STATUS_LABELS["not_announced"] if lineup_current else "Not checked"),
        ...
        if not lineup_current:
            st.info(
                "The validated browser-collected file for this week is not available yet. "
                "The GitHub workflow runs Thursday and Friday from 4:25 pm Perth time, with retries."
            )
        ...
        with st.expander("Paste AFL Team Line-ups fallback"):
            st.caption(
                "Use this only if the scheduled browser workflow has not produced a valid file. "
                "Open the AFL Team Line-ups page in your browser, select all team text and paste it here."
            )
            pasted_lineups = st.text_area(
                "Pasted team line-ups",
                height=220,
                key="pasted_afl_lineups",
                label_visibility="collapsed",
            )
            if st.button("Process pasted teams", use_container_width=True):
                try:
                    fixtures = list(cache.get("afl_matches") or [])
                    if not fixtures:
                        fixtures = AFLLineupClient(timeout=20).upcoming_matches(season)
                    pasted_result = result_from_pasted_text(
                        text=pasted_lineups,
                        season=season,
                        roster=ROSTER,
                        fixtures=fixtures,
                    ...
                    cache.update(pasted_result)
                    if pasted_result.get("afl_next_opponents"):
                        cache.setdefault("next_opponents", {}).update(pasted_result["afl_next_opponents"])
                    save_cache(cache, CACHE_PATH)
                    st.success("Pasted AFL teams processed.")
                    st.rerun()
                except Exception as exc:
                    st.error(str(exc))

        with st.expander("Manual status override"):
            st.caption("Use this for a late change or any player the automatic file has not classified correctly.")
    ...
    if lineup_refresh_clicked:
        st.divider()
        with st.spinner("Reloading the last validated AFL team line-up file…"):
            try:
                ...
                if result.get("lineup_failure_message"):
                    ...
                    st.success("Saved AFL team line-ups reloaded.")
                st.rerun()
            except Exception as exc:
                st.error(
                    "No usable validated AFL line-up file could be loaded. Existing statuses were retained. "
                    f"{exc}"
                )
