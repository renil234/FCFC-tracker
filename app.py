...
def main() -> None:
    ...
    from afl_lineups import AFLLineupClient, STATUS_LABELS, eligible_players, lineup_check_due
            ...
            starter_rows.append(
                {
                    ...
                    "Team status": (statuses.get(row["player"]) or {}).get(
                        "status_label", "Line-up check failed" if lineup_current else "Not checked"
                    ),
                }
            )
            ...
            roster_rows.append(
                {
                    ...
                    "Team status": status_record.get("status_label", "Line-up check failed" if lineup_current else "Not checked"),
            ...
            status_rows.append(
                {
                    "Player": player["player"],
                    "Club": player["club"],
                    "Status": record.get("status_label", "Line-up check failed" if lineup_current else "Not checked"),
